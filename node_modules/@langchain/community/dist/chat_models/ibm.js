...
import { WatsonxToolsOutputParser, _convertToolCallIdToMistralCompatible, authenticateAndSetInstance } from "../utils/ibm.js";
...
function _convertToValidToolId(model, tool_call_id) {
	if (model.startsWith("mistralai")) return _convertToolCallIdToMistralCompatible(tool_call_id);
...
function _convertDeltaToMessageChunk(delta, rawData, model, usage, defaultRole) {
	...
	else if (defaultRole) role = defaultRole;
	const content = delta.content ?? "";
	let additional_kwargs;
	if (rawToolCalls) additional_kwargs = { tool_calls: rawToolCalls };
	else additional_kwargs = {};
	if (role === "user") return new HumanMessageChunk({ content });
	else if (role === "assistant") {
		const toolCallChunks = [];
		if (rawToolCalls && rawToolCalls.length > 0) for (const rawToolCallChunk of rawToolCalls) toolCallChunks.push({
...
var ChatWatsonx = class extends BaseChatModel {
	...
	constructor(fields) {
		super(fields);
		if ("projectId" in fields && "spaceId" in fields || "projectId" in fields && "idOrName" in fields || "spaceId" in fields && "idOrName" in fields) throw new Error("Maximum 1 id type can be specified per instance");
		if ("model" in fields) {
	...
	invocationParams(options) {
		...
		const params = {
			...
			tools: options.tools ? _convertToolToWatsonxTool(options.tools) : void 0,
			responseFormat: options.responseFormat
		};
		const toolChoiceResult = options.tool_choice ? _convertToolChoiceToWatsonxToolChoice(options.tool_choice) : {};
		return {
			...params,
			...toolChoiceResult
		};
	...
	scopeId() {
		if (this.projectId && this.model) return {
			projectId: this.projectId,
			modelId: this.model
		};
		else if (this.spaceId && this.model) return {
			spaceId: this.spaceId,
			modelId: this.model
		};
		else if (this.idOrName) return { idOrName: this.idOrName };
		else if (this.model) return { modelId: this.model };
	...
	async _generate(messages, options, runManager) {
		if (this.streaming) {
			...
			const callback = () => "idOrName" in scopeId ? this.service.deploymentsTextChat({
				...scopeId,
				messages: watsonxMessages
			}, watsonxCallbacks) : this.service.textChat({
				...params,
				...scopeId,
				messages: watsonxMessages
			}, watsonxCallbacks);
			...
			return {
				generations,
				llmOutput: { tokenUsage: result?.usage }
			};
		}
	}
	async *_streamResponseChunks(messages, options, _runManager) {
		...
		const watsonxMessages = _convertMessagesToWatsonxMessages(messages, this.model);
		const watsonxCallbacks = this.invocationCallbacks(options);
		const callback = () => "idOrName" in scopeId ? this.service.deploymentsTextChatStream({
			...scopeId,
			messages: watsonxMessages,
			returnObject: true
		}, watsonxCallbacks) : this.service.textChatStream({
			...params,
			...scopeId,
			messages: watsonxMessages,
			returnObject: true
		...
		for await (const chunk of stream) {
			...
			if (message === null || !delta.content && !delta.tool_calls) continue;
	...
	withStructuredOutput(outputSchema, config) {
		...
		let llm;
		if (method === "jsonMode") {
			const options = { responseFormat: { type: "json_object" } };

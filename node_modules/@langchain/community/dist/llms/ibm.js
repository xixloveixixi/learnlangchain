...
var WatsonxLLM = class extends BaseLLM {
	...
	async generateSingleMessage(input, options, stream) {
		...
		if (stream) {
			const textStream = idOrName ? this.service.deploymentGenerateTextStream({
				idOrName,
				...requestOptions,
				parameters: {
					...parameters,
			...
			const textGenerationPromise = idOrName ? this.service.deploymentGenerateText({
				...requestOptions,
				idOrName,
				parameters: {
					...parameters,
	...
	async *_streamResponseChunks(prompt, options, runManager) {
		...
		for await (const chunk of streamInferDeployedPrompt) {
			...
			for (const [index, item] of chunk.data.results.entries()) {
				...
				if (!this.streaming) runManager?.handleLLMNewToken(item.generated_text);
			}
